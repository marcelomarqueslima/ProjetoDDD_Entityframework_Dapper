...
namespace UnitTest.Integration.Repositories.Repositories.Dapper
{
    ...
    public class UserRepositoryTransactionTest
    {
        ...
        public void Inicializa()
        {
            ...
            taskToDoDapper = new TasksToDoRepository(conn, transaction);
        ...
        public async Task GetAllIncludingTasksAsync()
        {
            var user1 = await userDapper.AddAsync(userBuilder.CreateUser());
            var user2 = await userDapper.AddAsync(userBuilder.CreateUser());
            var task1 = await taskToDoDapper.AddRangeAsync(taskToDoBuilder.CreateTasksToDoListWithUser(1, user1.Id));
            var task2 = await taskToDoDapper.AddRangeAsync(taskToDoBuilder.CreateTasksToDoListWithUser(2, user2.Id));
        ...
        {
            var user = await userDapper.AddAsync(userBuilder.CreateUser());
            var task = await taskToDoDapper.AddRangeAsync(taskToDoBuilder.CreateTasksToDoListWithUser(3, user.Id));
