...
namespace Infrastructure.Repositories.Domain.Dapper
{
                              ...
                              IUserRepository
    {
        ...
        private string SelectAllIncludingRelation => $"SELECT u.*, t.* FROM [{nameof(User)}] u LEFT JOIN [{nameof(TasksToDo)}] t ON t.{nameof(TasksToDo.UserId)} = u.{nameof(User.Id)}";
        private string SelectByIdIncludingRelation => $"SELECT u.*, t.* FROM [{nameof(User)}] u LEFT JOIN [{nameof(TasksToDo)}] t ON t.{nameof(TasksToDo.UserId)} = u.{nameof(User.Id)} WHERE u.{nameof(User.Id)} = @{nameof(User.Id)}";

        public async Task<IEnumerable<User>> GetAllIncludingTasksAsync()
        {
            var userDictionary = new Dictionary<int, User>();
            var queryResult = await dbConn.QueryAsync<User, TasksToDo, User>(SelectAllIncludingRelation, transaction: dbTransaction,
        ...
        {
            var userDictionary = new Dictionary<int, User>();
            var queryResult = await dbConn.QueryAsync<User, TasksToDo, User>(SelectByIdIncludingRelation, param: new { Id = id }, transaction: dbTransaction,
                map: (user, toDoList) => FuncMapRelation(user, toDoList, userDictionary));

            return queryResult.Distinct().FirstOrDefault();
        }

        private readonly Func<User, TasksToDo, Dictionary<int, User>, User> FuncMapRelation = (user, tasksToDo, userDictionary) =>
        {
